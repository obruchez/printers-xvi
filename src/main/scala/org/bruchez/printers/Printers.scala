...
object Printers {
	...
	def shortName(name: String): String = {
	  ...
	  firstWord.filterNot(word => word.endsWith(".") || word.size < 1 || isLowerCase(word.substring(0, 1).charAt(0))) map { word =>
    ...
    def unitlessCount(s: String): Option[Int] =
	  ...
	  RawCount.findFirstMatchIn(html).map(_.group(1)) map { rawCount =>	    
	    ...
	    rawCount.
	      ...
	      replace("[8] 343 [1] : 431, [1] : 348 : 272 : [8], 340, [36] : [56] f.", "8, 343, 1 : 431, 1 : 348 : 272 : 8, 340, 36 : 56 f.").
          replace("[8], 362 : 81, [1] : 108, [22] : 32  [18] f.", "8, 362 : 81, 1 : 108, 22 : 32, 18 f.").
          replace("[8] 862 [= 855], [1] : 194 [= 201], [1] : [10], 299 [= 270], [52] f.", "8, 862, 1 : 194, 1 : 10, 299, 52 f.").
